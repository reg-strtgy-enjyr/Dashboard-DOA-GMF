...
export class FollowonIORComponent {
  ...
  async getAccountInfo() {
    try {
      const response = await axios.post('http://34.46.32.81:4040/account/show', { accountid: this.currentAccountID });
  ...
  async submitFollowIOR() {
    ...
    try {
      const response = await axios.post('http://34.46.32.81:4040/ior/follow-up/add', this.followIORData);
