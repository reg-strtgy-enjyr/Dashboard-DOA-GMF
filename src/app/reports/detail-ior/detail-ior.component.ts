...
export class DetailIORComponent implements OnInit{
  ...
  async fetchIOR() {
    try {
      const response = await axios.post('http://34.132.47.129:4040/ior/show',
  ...
  async fetchFollowonIOR() {
    try {
      const response = await axios.get('http://34.132.47.129:4040/ior/follow-up/show-all');
  ...
  async navigatePreview() {
    ...
    try {
      const response = await axios.post('http://34.132.47.129:4040/ior/getPDF', 
        { id_IOR: this.currentIORID }
      );
      if (response.data.status === 200) {
