...
export class EditFollowonIORComponent implements OnInit{
  ...
  async fetchFollowupIOR() {
    try {
      const response = await axios.post('http://34.46.32.81:4040/ior/follow-up/show',
  ...
  async updateFollowupIOR() {
    ...
    try {
      const response = await axios.put('http://34.46.32.81:4040/ior/follow-up/update', this.followonData);
