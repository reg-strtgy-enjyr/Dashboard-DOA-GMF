...
export class SearchIORComponent implements OnInit {
  ...
  async fetchDataFromServer() {
    try {
      const response = await axios.get('http://34.46.32.81:4040/ior/show-all');
  ...
  async fetchDataBySearchTerm() {
    try {
      const response = await axios.post('http://34.46.32.81:4040/ior/search', { input: this.searchTerm });
      if (response.data.status === 200) {
